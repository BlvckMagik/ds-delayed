...
export class TelegramService {
  ...
  private async sendLessonReminder(lesson: Lesson) {
    ...
    try {
      ...
      this.logger.log(
        `Sent reminder for lesson: ${lesson.name} to chat: ${lesson.group.chatId}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to send reminder for lesson: ${lesson.name}`,
        error,
      );
    }
  }
  ...
  private async createLessonMessage(lesson: Lesson): Promise<string> {
    ...
    const berlinTime = formatInTimeZone(now, 'Europe/Berlin', 'HH:mm', {
      locale: uk,
    });
